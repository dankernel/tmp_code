...
{/*{{{*/
  ...
  if (cm->max > cm->size) {
    ...
    if (!lookup) {
      ...
      list_add(&new->head, cm->list);
      lookup = container_of(cm->list->next, struct cache_line, head);
      list_del(cm->list->next);
      // print_cm(cm);
...
{/*{{{*/
  ...
  // printf("%d %20s %15ld %10.2f %10ld %5.2f\n", wl->type, wl->time, wl->offset, start, wl->size, count);
   ...
   do {
    ...
    if (wl->type == READ) {
      LRU_read(cm, start + i);
        // printf("R >> %.2fBlock. %.2f, %.2f \n", start + i, wl->offset, wl->size);
    } else if (wl->type == WRITE) {
      LRU_write(cm, start + i);
        // printf("W >> %f\n", start + i);
    }
